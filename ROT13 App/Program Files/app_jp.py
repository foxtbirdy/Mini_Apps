import sys
from PyQt5.QtWidgets import *
from PyQt5.QtGui import *      
from PyQt5.QtCore import *
import pyperclip                    


__author__ = "@Black_2_white"
__encryption_by__ = "sam allen"


class Example(QMainWindow):


    def __init__(self):
        super().__init__()
        self.setWindowTitle("Rot13 Program")
        self.setFixedSize(800,840)
        self.box_edit()
        self.button_controls()
        self.widget_tip()
        self.slot_connect()
        self.setStyleSheet(open("design.css", "r").read())
        self.set_label_logo()

    def button_controls(self):
        self.button_paste = QPushButton(self)
        self.button_paste.setText("実行する")
        self.button_paste.clicked.connect(self.paste_text)
        self.button_paste.move(150,350)
        
        self.button_copy = QPushButton(self)
        self.button_copy.setText("コピー")
        self.button_copy.clicked.connect(self.copy_text)
        self.button_copy.move(30,350)
        
        self.button_clear = QPushButton(self)
        self.button_clear.setText("晴れ")
        self.button_clear.clicked.connect(self.clear_text)
        self.button_clear.move(270,350)
           
            
    def box_edit(self):
        self.text_edit = QTextEdit(self)
        self.text_edit.setPlaceholderText("テキストはここにあります")
        self.text_edit.setGeometry(30,30, 740,300)
        self.text_edit.textChanged.connect(self.text_changed)
        self.text_edit.setStyleSheet("""
    
        background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                                      stop:0 #bcbfdb, stop:1 #858fdf)
        """)
        self.text_edit_2 = QTextEdit(self)
        self.text_edit_2.setPlaceholderText("ここに翻訳されたテキスト")
        self.text_edit_2.setGeometry(30,400, 740,300)
        self.text_edit_2.setReadOnly(True)
        
        self.text_edit_2.setStyleSheet("""
        background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                                      stop:0 #e6a8d9, stop:1 #ff00bf)
        """)
        
        
    def widget_tip(self):
        self.button_copy.setToolTip("<b>Copy Text</b>")
        self.button_paste.setToolTip("Paste <b>Copied </b> text")
        self.button_clear.setToolTip("Clear <b>Text</b>")
    
    def slot_connect(self):
        self.text_edit.textChanged.connect(self.text_changed)
        self.button_paste.clicked.connect(self.paste_text)
        self.button_copy.clicked.connect(self.copy_text)
        self.button_clear.clicked.connect(self.clear_text)

    
    def set_label_logo(self):
        self.logo = QLabel(self)
        self.logo.setStyleSheet("""
        background-image: url(icon.png)
        """)
        self.logo.setGeometry(560,760,62,62)
        
        self.text_label = QLabel(self)
        self.text_label.setText("ROT13 \nGUI \nAPPLICATION")
        self.text_label.setGeometry(630, 755, 140, 70)

        
        
    def rot13(self):
        result = ""
        s = self.text_edit.toPlainText()
        for v in s:
            c = ord(v)
            if c >= ord('a') and c <= ord('z'):
                if c > ord('m'):
                    c -= 13
                else:
                    c += 13
            elif c >= ord('A') and c <= ord('Z'):
                if c > ord('M'):
                    c -= 13
                else:
                    c += 13
            result += chr(c)
        printout = self.text_edit_2.setText("".join(result)) # Printout!
        return printout
        
        
    def copy_text(self):
        if self.text_edit.toPlainText() == '':
            pass
        elif self.text_edit.toPlainText() == '\n':
            pass
        else:
            pyperclip.copy(self.text_edit.toPlainText())

        
    def clear_text(self):
        self.text_edit_2.clear()
        self.text_edit.clear()
    
    
    def paste_text(self):
        self.text_edit.setText(pyperclip.paste())

        
    def text_changed(self):
        self.rot13()
        
        
if __name__ == '__main__':
    app = QApplication(sys.argv)
    ex = Example()
    ex.show()
    sys.exit(app.exec_())